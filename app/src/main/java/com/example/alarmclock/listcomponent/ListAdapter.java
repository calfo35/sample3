...
public class ListAdapter extends RecyclerView.Adapter<ListViewHolder> {
    ...
    public void onBindViewHolder(@NonNull ListViewHolder holder, int position) {
        ListItem currentItem = this.data.get(position);

        // アラーム名と時刻をセット
        holder.alarmName.setText(currentItem.getAlarmName());
        holder.time.setText(currentItem.getTime());

        // 朝活の種類と勉強時間をセット
        holder.activityType.setText(currentItem.getActivityType()); // 朝活の種類
        holder.studyTime.setText(currentItem.getStudyTime() + "分"); // 勉強時間
    }
